...
represent which number is marked as visited so far in the current number). Start with state (0,0,0) and count all valid number till we 
reach number of steps equals to 10n.
...
{
	Solution sol;
	int i = sol.countNumbersWithUniqueDigits(1);
	cout << i << endl;
	return 0;
}